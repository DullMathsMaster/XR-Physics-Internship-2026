...
namespace LightTK
{
    ...
    public partial class LTK
    {
        ...
        public static bool SolveRayIdealLens(LightRay l, LightRayHit p)
        {
            ...
            float focalLength = sign > 0
                ? p.surface.settings.focalLength(l)
                : -p.surface.settings.focalLength(l);

            Vector3 relDir = Quaternion.Inverse(p.surface.rotation) * l.direction;
            Vector3 hit = new Vector3(
                focalLength / relDir.z * relDir.x,
                focalLength / relDir.z * relDir.y,
                focalLength);

            l.direction = p.surface.rotation * hit + p.surface.position - p.point;

            if (p.surface.settings.invR1 < 0 && p.surface.settings.invR2 > 0)
            {
                l.direction *= -1f;
            }
        ...
        public static bool SimulateRay(LightRay l, List<LTKCollider> curves, out LightRayHit hit)
        {
            ...
            for (int i = 0; i < curves.Count; i++)
            {
                if (!curves[i].enableCollision) continue;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static bool SimulateRay(LightRay l, Surface[] curves) => SimulateRay(l, curves, out _);

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static bool SimulateRay(LightRay l, List<Surface> curves) => SimulateRay(l, curves, out _);

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static bool SimulateRay(LightRay l, List<LTKCollider> curves) => SimulateRay(l, curves, out _);

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static bool SimulateRay(LightRay l, LTKCollider[] curves) => SimulateRay(l, curves, out _);
        ...
        public static void SimulateRays(List<LightRay> rays, List<LTKCollider> curves)
        {
            for (int i = 0; i < rays.Count; i++)
            {
                LightRay l = rays[i];
